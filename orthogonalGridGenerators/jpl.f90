...
program jpl
	...
	parameter (nn=50000,n1=2,n2=4,n3=18,n4=8,n5=3,n6=95,nh=50,dnk=6.0d0,dnkconv=1.15)
	...
	print*,x0
	do i =1,n1+1
		xw(i)=x0+dfloat(i-(1))*dx
		yw(i)=h1
	enddo

	da=ang1/dfloat(n2)
	do i=n1+2,n1+n2+1
		dx=r1*dsin(da*dfloat(i-(n1+1)))
		dy=-r1*(1.0d0-dcos(da*dfloat(i-(n1+1))))
		xw(i)=xw(n1+1)+dx
		yw(i)=yw(n1+1)+dy
	enddo
	...
	do i=n1+n2+2,n1+n2+n3+1
		x0=xw(n1+n2+1)
		y0=yw(n1+n2+1)
		eta=dfloat(i-(n1+n2+1))/dfloat(n3)
	...
	do i=n1+n2+n3+2,n1+n2+n3+n4+1
		x0=xw(n1+n2+n3+1)
		y0=yw(n1+n2+n3+1)
		xw(i)=x0+rc*(dsin(ang1)-dsin(ang1-da*dfloat(i-(n1+n2+n3+1))))
		yw(i)=y0+rc*(dcos(ang1)-dcos(ang1-da*dfloat(i-(n1+n2+n3+1))))
	enddo
	da=ang2/dfloat(n5)
	do i=n1+n2+n3+n4+2,n1+n2+n3+n4+n5+1
		x0=xw(n1+n2+n3+n4+1)
		y0=yw(n1+n2+n3+n4+1)
		xw(i)=x0+rc*dsin(da*dfloat(i-(n1+n2+n3+n4+1)))
		yw(i)=y0+rc*(1.0d0-dcos(da*dfloat(i-(n1+n2+n3+n4+1))))
	enddo
	dx=(dist3-dist2-rc*dsin(ang1)-rc*dsin(ang2))/dfloat(n6)
	do i=n1+n2+n3+n4+n5+2,n1+n2+n3+n4+n5+n6+1
		x0=xw(n1+n2+n3+n4+n5+1)
		y0=yw(n1+n2+n3+n4+n5+1)
		xw(i)=x0+dx*dfloat(i-(n1+n2+n3+n4+n5+1))
...
98			format(5i8)
		enddo
	enddo	
	print*,'near wall grid size: ',y(ny)-y(ny-1),' mm'
	close(21)
	close(22)
endprogram jpl
